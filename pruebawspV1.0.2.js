...
const { saveExternalFile } = require('../bot-whatsapp-main/controllers/handle');
const app = express();
    ...
    if (msg.body == 'Quiero info'){
        ...
        if(chat_id.includes(msg.from)){
            ...
            if (body == "Ver resultados"){
                ...
                sec_busc = false;
                var body_vect = Array.from(body);
                var palabras = ['','','','','','']
                var cont_palabras = 0;
                body_vect.forEach(letra => {
                    if(letra == " "){
                        cont_palabras++; 
                    }
                    else if(letra == "á"){
                        palabras[cont_palabras]= palabras[cont_palabras] + "a";    
                    }
                    else if(letra == "é"){
                        palabras[cont_palabras]= palabras[cont_palabras] + "e";    
                    }
                    else if(letra == "í"){
                        palabras[cont_palabras]= palabras[cont_palabras] + "í";    
                    }
                    else if(letra == "ó"){
                        palabras[cont_palabras]= palabras[cont_palabras] + "o";    
                    }
                    else if(letra == "ú"){
                        palabras[cont_palabras]= palabras[cont_palabras] + "u";    
                    }
                    else{
                        palabras[cont_palabras]= palabras[cont_palabras] + letra; 
                    }
                });
                //busquedaExcel(from,body);
                for (var i = 0; i <= cont_palabras; i++){
                    sendMessage(from,palabras[i])
                }
...
function busquedaExcel(from,el_valor) {
    ...
    dataExcel.forEach(j => {
      ...
      if(cant_selec > 20 ){
        sendMessage(from,"Se encontraron demasiados resultados, detalle mejor su busqueda.");
      }

      else if(cant_selec == 0) {
        sendMessage(from,"Lo lamento. No pude encontrar articulos con ese nombre. Prueba con otro.");
      }
